...
async function handleEmailSubmit() {
    ...
    try {
        if (mode.value === 'login') {
            ...
            // Para registro, usamos a action diretamente (extensível)
            const { registerWithEmail } = await import('@/services/firebase')
            await registerWithEmail(email.value, password.value)
        }
        router.push({ name: 'dashboard' })
    } catch {
        // Erro já foi capturado pelo store (authStore.error)
    } finally {
        ...
        <div class="w-full max-w-md animate-fade-in">
            ...
            <div class="bg-white dark:bg-slate-900 rounded-2xl shadow-sm border border-slate-100
                ...
                <Transition name="fade">
                    <div v-if="authStore.error" class="mb-5 p-3 bg-red-50 dark:bg-red-950 border border-red-100 dark:border-red-900
                ...
                <form class="space-y-4" @submit.prevent="handleEmailSubmit">
                    ...
                    <div>
                       ...
                       dark:hover:text-slate-300 transition-colors" @click="showPassword = !showPassword">
                    ...
                    <!-- Submit button -->
                    <button type="submit" :disabled="isLoading" class="w-full py-2.5 px-4 bg-primary-600 hover:bg-primary-700 disabled:bg-primary-300
                   text-white font-medium rounded-xl transition-colors duration-200 text-sm
                   focus:outline-none focus:ring-2 focus:ring-primary-500 focus:ring-offset-2
                   dark:focus:ring-offset-slate-900 flex items-center justify-center gap-2">
                ...
                <!-- Google login -->
                <button :disabled="isLoading" class="w-full flex items-center justify-center gap-3 py-2.5 px-4
                 border border-slate-200 dark:border-slate-700 rounded-xl
                 text-slate-700 dark:text-slate-300 hover:bg-slate-50 dark:hover:bg-slate-800
                 transition-colors duration-200 text-sm font-medium" @click="handleGoogleLogin">
                    <!-- Google logo SVG -->
                ...
                <p class="text-center text-sm text-slate-500 dark:text-slate-400 mt-6">
                    ...
                    <button class="text-primary-600 dark:text-primary-400 font-medium hover:underline ml-1"
